...
| --- | --- |
| `block_ranges` | **the field that makes the object source-agnostic** — declares the Matryoshka block boundaries, so the metrics never need to know which model produced the dictionary |
...
1. Emit `full`. Set `config.block_ranges` from your SAE's own nesting schedule — not from
